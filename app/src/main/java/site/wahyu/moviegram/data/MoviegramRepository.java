...
public class MoviegramRepository {

    private volatile static MoviegramRepository INSTANCE = null;

    private final LocalRepository localRepository;
    private final RemoteRepository remoteRepository;
    private final AppExecutors appExecutors;

    private MoviegramRepository(@NonNull LocalRepository localRepository, @NonNull RemoteRepository remoteRepository, AppExecutors appExecutors) {
        this.localRepository = localRepository;
        this.remoteRepository = remoteRepository;
        this.appExecutors = appExecutors;
    }

    public static MoviegramRepository getInstance(LocalRepository localRepository, RemoteRepository remoteData, AppExecutors appExecutors) {
        if (INSTANCE == null) {
            synchronized (MoviegramRepository.class) {
                if (INSTANCE == null) {
                    INSTANCE = new MoviegramRepository(localRepository, remoteData, appExecutors);
                }
            }
        }
        return INSTANCE;
    ...
    public LiveData<Resource<PagedList<FavoriteMovieEntity>>> filterFavoriteMovies(String sort){
        return new NetworkBoundResource<PagedList<FavoriteMovieEntity>, List<Movie>>(appExecutors) {
            @Override
            public LiveData<PagedList<FavoriteMovieEntity>> loadFromDB() {
                return new LivePagedListBuilder<>(localRepository.filterFavoriteMovie(sort), 1).build();
            }
